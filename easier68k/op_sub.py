...
class OpCodeSub(OpCodeAdd):
    ...
    def execute(self, cpu: M68K):
        ...
        if self.direction == 1:
            # ea - Dn -> ea
            final_val, carry, overflow = ea_val.sub_unsigned(dn_val)
            print(f"1 storing {final_val} in ea")
            self._set_ea_mode_value(self.size, cpu, final_val)
        else:
            # Dn - ea -> Dn
            final_val, carry, overflow = dn_val.sub_unsigned(ea_val)
            print(f"0 storing {final_val} in dx")
            cpu.set_register(self.data_register, final_val)

        cpu.set_ccr_reg(None, final_val.get_negative(), final_val.get_zero(), overflow, carry)
